...
def get_response_stream(session_log, settings, web_search_enabled=False, search_results=None,
    ...
    current_line = ""           # <-- NEW: track current line for INST detection
    ...
    for chunk in llm_state.create_chat_completion(
        ...
        # *** NEW: Detect [INST] or </INST> at start of new line ***
        current_line += token
        ...
        if '\n' in current_line:
            ...
            for i, line in enumerate(lines[:-1]):
                # Check if line starts with INST tags (case insensitive)
                if (line.strip().lower().startswith('[inst]') or 
                    line.strip().lower().startswith('[/inst]') or
                    line.strip().lower().startswith('<inst>') or
                    line.strip().lower().startswith('</inst>')):
                    
                    # Stop output immediately - don't yield this line or anything after
                    return
                
                # Yield the line with its newline
                if i == 0:  # First line
                    yield line + '\n'
                else:  # Subsequent lines start with newline
                    yield '\n' + line
            
            # Keep the incomplete line for next iteration
            current_line = lines[-1] if lines[-1] else ""
        
        # If no newline yet, just yield the token (for immediate feedback)
        elif not any(tag in current_line.strip().lower() for tag in ['[inst]', '[/inst]', '<inst>', '</inst>']):
            yield token
    
    # Handle any remaining content in current_line
    if current_line.strip():
        # Final check for INST tags at start of remaining line
        if not (current_line.strip().lower().startswith('[inst]') or 
                current_line.strip().lower().startswith('[/inst]') or
                current_line.strip().lower().startswith('<inst>') or
                current_line.strip().lower().startswith('</inst>')):
            yield current_line
