...
def launch_interface():
    ...
    with gr.Blocks(title="Chat-Gradio-Gguf", css=".scrollable{overflow-y:auto}.send-button{background-color:green!important;color:white!important;height:80px!important}.double-height{height:80px!important}.clean-elements{gap:4px!important;margin-bottom:4px!important}.clean-elements-normbot{gap:4px!important;margin-bottom:10px!important}") as demo:
        # States
        states = dict(loaded_files=gr.State([]), models_loaded=gr.State(False), showing_rpg_right=gr.State(False), cancel_flag=gr.State(False))
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        with gr.Column(visible=True, elem_classes=["clean-elements"]):
                            buttons = dict(start_new_session=gr.Button("Start New Session...", variant="secondary"),
                    ...
                    with gr.Column(scale=30, elem_classes=["clean-elements"]):
                        chat_components = dict(session_log=gr.Chatbot(label="Session Log", height=temporary.SESSION_LOG_HEIGHT, elem_classes=["scrollable"], type="messages"),
                        ...
                        with gr.Row(elem_classes=["clean-elements"]):
                            ...
                            with gr.Column(elem_classes=["clean-elements"]):
                                action_buttons.update(send=gr.Button("Send Input", variant="secondary", scale=20, elem_classes=["send-button"]),
                                                      cancel=gr.Button("Cancel Input", variant="stop", scale=20, visible=False, elem_classes=["double-height"]))
                            with gr.Column(elem_classes=["clean-elements"]):
                                with gr.Row(elem_classes=["clean-elements"]):
                                    action_buttons.update(edit_previous=gr.Button("Edit Last Input", variant="huggingface"),
                                                          copy_response=gr.Button("Copy Last Output", variant="huggingface"))
                                with gr.Row(elem_classes=["clean-elements"]):
                                    switches = dict(web_search=gr.Checkbox(label="Web-Search", value=False, visible=True),
                    ...
                    with gr.Column(scale=1):
                        right_panel = dict(mode_selection=gr.Radio(choices=["Chat", "Code", "Rpg"], label="Select Operation Mode", value="Chat"),
                        ...
                        with right_panel["file_attachments"]:
                            right_panel.update(attach_files=gr.UploadButton("Attach New Files", file_types=[f".{ext}" for ext in temporary.ALLOWED_EXTENSIONS], file_count="multiple", variant="secondary", elem_classes=["clean-elements"]),
                        ...
                        with right_panel["rpg_settings"]:
                            rpg_fields = dict(rp_location=gr.Textbox(label="RP Location", value=temporary.RP_LOCATION),
            ...
            with gr.Tab("Configuration"):
                with gr.Column(scale=1, elem_classes=["clean-elements"]):
                    ...
                    with gr.Row(visible=not is_cpu_only, elem_classes=["clean-elements"]):
                        config_components.update(gpu=gr.Dropdown(choices=utility.get_available_gpus(), label="Select GPU", value=temporary.SELECTED_GPU, scale=5),
                    ...
                    with gr.Row(visible=is_cpu_only, elem_classes=["clean-elements"]):
                        ...
                        config_components.update(backend_type=gr.Textbox(label="Backend Type", value=temporary.BACKEND_TYPE, interactive=False, scale=5),
                    ...
                    with gr.Row(elem_classes=["clean-elements"]):
                        config_components.update(model_dir=gr.Textbox(label="Model Folder", value=temporary.MODEL_FOLDER, interactive=False, scale=20),
                                                 browse=gr.Button("Browse", variant="secondary", elem_classes=["double-height"]))
                    with gr.Row(elem_classes=["clean-elements"]):
                        config_components.update(model=gr.Dropdown(choices=available_models, label="Select Model", value=default_model, allow_custom_value=True, scale=10),
                    ...
                    with gr.Row(elem_classes=["clean-elements"]):
                        config_components.update(ctx=gr.Dropdown(choices=temporary.CTX_OPTIONS, label="Context Size", value=temporary.N_CTX),
                    ...
                    with gr.Row(elem_classes=["clean-elements"]):
                        config_components.update(load_models=gr.Button("Load Model", variant="secondary", elem_classes=["double-height"]),
                    ...
                    custom_components = {}
                    with gr.Row(elem_classes=["clean-elements"]):
                        custom_components.update(max_history_slots=gr.Dropdown(choices=temporary.HISTORY_SLOT_OPTIONS, label="Max History Slots", value=temporary.MAX_HISTORY_SLOTS),
                    ...
                    with gr.Row(elem_classes=["clean-elements"]):
                        config_components.update(save_settings=gr.Button("Save Settings", variant="primary", elem_classes=["double-height"]))
                    ...
                    with gr.Row(elem_classes=["clean-elements"]):
                        config_components.update(status_settings=gr.Textbox(label="Status", interactive=False, scale=20),
        ...
        def update_mlock(mlock): temporary.MLOCK = mlock; return "MLock updated."
        def save_all_settings(): utility.save_config(); return "Settings saved to persistent.json"
        ...
        action_buttons["send"].click(fn=chat_interface, inputs=[chat_components["user_input"], chat_components["session_log"], switches["tot"], states["loaded_files"], switches["disable_think"], rpg_fields["rp_location"], rpg_fields["user_name"], rpg_fields["user_role"], rpg_fields["ai_npc"], states["cancel_flag"], right_panel["mode_selection"], switches["web_search"]], outputs=[chat_components["session_log"], status_text, action_buttons["send"], action_buttons["cancel"], states["cancel_flag"], states["loaded_files"]])
        ...
        right_panel["attach_files"].upload(fn=process_uploaded_files, inputs=[right_panel["attach_files"], states["loaded_files"], states["models_loaded"]], outputs=[status_text, states["loaded_files"]])
        ...
        for i, btn in enumerate(right_panel["file_slots"]): btn.click(fn=eject_file, inputs=[states["loaded_files"], gr.State(value=i)], outputs=[states["loaded_files"], status_text] + right_panel["file_slots"] + [right_panel["attach_files"]])
        for comp in [config_components[k] for k in ["ctx", "batch", "temp", "repeat", "vram", "gpu", "cpu", "model"]]: comp.change(fn=update_config_settings, inputs=[config_components[k] for k in ["ctx", "batch", "temp", "repeat", "vram", "gpu", "cpu", "model"]] + [config_components["model_dir"]], outputs=[config_components["status_settings"]])
        config_components["browse"].click(fn=select_model_folder, outputs=[config_components["model_dir"]]).then(fn=update_model_list, inputs=[config_components["model_dir"]], outputs=[config_components["model"]]).then(fn=lambda f: f"Model directory updated to: {f}", inputs=[config_components["model_dir"]], outputs=[config_components["status_settings"]])
        for chk in [config_components.get(k) for k in ["mlock_gpu", "mlock_cpu"] if config_components.get(k)]: chk.change(fn=update_mlock, inputs=[chk], outputs=[config_components["status_settings"]])
        config_components["unload"].click(fn=unload_models, outputs=[config_components["status_settings"]]).then(fn=lambda: False, outputs=[states["models_loaded"]])
        config_components["inspect_model"].click(fn=inspect_model, inputs=[config_components["model"], config_components["vram"]], outputs=[config_components["status_settings"]])
        ...
        right_panel["toggle_rpg_settings"].click(fn=toggle_rpg_settings, inputs=[states["showing_rpg_right"]], outputs=[right_panel["toggle_rpg_settings"], right_panel["file_attachments"], right_panel["rpg_settings"], states["showing_rpg_right"]])
        rpg_fields["save_rpg"].click(fn=save_rp_settings, inputs=list(rpg_fields.values())[:-1], outputs=list(rpg_fields.values())[:-1])
        custom_components["delete_all_vectorstores"].click(fn=utility.delete_all_session_vectorstores, outputs=[config_components["status_settings"]])
        ...
        custom_components["afterthought_time"].change(fn=lambda v: setattr(temporary, "AFTERTHOUGHT_TIME", v), inputs=[custom_components["afterthought_time"]], outputs=[config_components["status_settings"]])
        config_components["model"].change(fn=update_model_based_options, inputs=[config_components["model"]], outputs=[right_panel["mode_selection"], switches["disable_think"], recommended_mode]).then(fn=update_mode_based_options, inputs=[right_panel["mode_selection"], states["showing_rpg_right"]], outputs=[switches["tot"], switches["web_search"], right_panel["attach_files"], right_panel["toggle_rpg_settings"], right_panel["file_attachments"], right_panel["rpg_settings"], states["showing_rpg_right"]])
        right_panel["mode_selection"].change(fn=update_mode_based_options, inputs=[right_panel["mode_selection"], states["showing_rpg_right"]], outputs=[switches["tot"], switches["web_search"], right_panel["attach_files"], right_panel["toggle_rpg_settings"], right_panel["file_attachments"], right_panel["rpg_settings"], states["showing_rpg_right"]])
