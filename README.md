...
### Status
Beta - Unfortunately TTS had to be removed; there is no way to have consistent libraries over the range of OS intended to be covered, resulting in highly complicated tool install/configuration, with fallbacks to non Corqui slipping in, on-top of the total program complexity and the ~2500 line installer as it is. The TTS and/or more tools, will not be attempted again until claude is v5. Soon there will be v1.00.0, a fully working version on both windows and ubuntu, after a little cleanup.
