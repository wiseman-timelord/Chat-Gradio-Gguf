...
### Status
Beta - Unfortunately TTS had to be removed; there is no way to have consistent libraries over the range of OS intended to be covered, resulting in highly complicated tool install/configuration, with fallbacks to non Corqui slipping in, on-top of the total program complexity and the ~2500 line installer as it is. The TTS and/or more tools, will not be attempted again until claude is v5. Soon there will be v1.00.0, a fully working version on both windows and ubuntu, after a little cleanup. Other project will begin "Chat-LightWave-Gguf" a windows 8.1-11 project using Edge Webview with gradio v5, where there will be room for a few more tools. Possibly if I dropped support for Ubuntu 22-23 and windows 7-8.1 for version 1.01.0+, then there could be more room, I could do that, but it would require me to use windows 10, and I have my issues, issues that would go away if github/x dropped support of edge based on chromium v109, but they have not done that yet.
...
### Preview
- The Interaction page on Windows, in this case showing the "Hybrid DDG Search", with Command Prompt in the background (v0.99.8_r2)...
