...
<br>Status: Beta - Working fine recently up until ~93.5, then there was an update to gradio making it 4.44.1, and since then there is some massive issue, that could not be fixed in 2.5 sessions in public spaces, so, for now, progress is halted. Work is needed to be completed on prompting, as harmony/moe/code require no system prompt/pre-prompt.
