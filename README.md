...
### Status
Beta - Latest release v1.04.x now includes "About/Debug Info" tab with contact/support info. See release notes. Work on the Project has halted for now, so focus can return to other things. It has also become apparent that, the program is so complicated now, that unless I have multiple OS setup, then I cannot bugfix it correctly for all case uses, as its a MASSIVE challenge for AI to program with case modes for the intended OS ranges. Possible I could also continue work when AI advances a bit, however I will be focusing on windows 10-11 project restart of Agent-Electron-Gguf.
