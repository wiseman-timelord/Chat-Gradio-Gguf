...
### Status
Beta - Latest release v1.10.9 was completion of overhaul, and tested working with Qwen 3.5/3.6 on windows 10, however it turn out other models are now not working, so I'd wait for next release for version that can run both newer and older models, as the newer models are the focus. Testing/bugfixing on Ubuntu needs to be done, but past releases mention ubuntu/linux compatibility.
