...
<br>Status: Beta - Working good now, however main issues; 1), no cancel response button, do not quit while wait response with file attached, or must delete history. 2) The update to attachments seems to have broken the rag in relevance to unlimited user input length. So if you dont want to use gpt-oss, then you are advised to use v0.92.x, until this is fixed.
