...
### Status
Beta - Working fine on v0.99.x on my Ubuntu 25, needs to be re-checked on Windows 8.1 system, after the recent updates to enable compatibility with Ubuntu, as there were a few, which shows how many updates/upgrades I did recently in 8.1. If 0.99.x dont work on windows then try 0.98.x.
...
- Llm Model - You will need a Large Language Model in GGUF format, check the models section for recommendations, but for quick start I advise one like this [Qwen3-4B-abliterated-GGUF](https://huggingface.co/mradermacher/Qwen3-4B-abliterated-GGUF) for testing basic operation.
...
2. Yous should click the `Configuration` tab. On the `Configuration` page you would configure appropriately, its all straight forwards, but, take your time and remember to save settings, and then load model. If the model loads correctly it will say so in the `Status Bar` on the bottom od the display.
