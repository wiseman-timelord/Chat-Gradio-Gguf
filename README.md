...
### Preview
- The main chat window after a date aware AI Web Search report, where both side panels are visible (using the model Qwen3) (v2.00)...
...
- The About/Debug/Info page, displaying useful information (v2.00.5)...
...
5. When there is a stable correctly working latest v2, then the plan will be to develop some extra tools, ie "Web Research", but this needs to be brainstormed.
7. When the user installs, the installer needs to additionally note what version of the wheel was installed, currently that would be either v0.3.20 or v0.3.16, though possibly there is a global for this, but it needs to additionally be noted into the constants.ini, in order to be able to display this in the appropriate location in the about/debug tab in the main program, for purposes of referencing what level of model compatibility the program was installed with.
8. Code Optimization / redistribution of code, moving code out from display script, to applicably relatingly themed scripts, if there is relevance to both scripts and it is sound to do so, in order to reduce overall code in display script by moving it out, as appropriate to do so, nothing extreme. 
9. It would be great to have a Auto/Quick/Think toggle/features, it would help with agentic operation tasks. We need an assessment of which of the top models would be able to handle the tasks through llama.cpp, and program towards compatibility with only those models. We would have a pre-prompt that would assess the input from the user, with, a temporary set context 32768 and batch output 2048, from which it would assess the best, context size and batch size, for the workload, based on how complicated the task is and what kind of response output is likely expected. In addition to this we meed the AI to decide which of the attached scripts need to be included in the prompt that is read, this is instead of just inserting all scripts in-line as soon as they are attached. So when attached, the submitted scripts/documents would be a resource in the specific session, within the folder for the relating specific session, that in future the AI can call upon for reference at any point, due to its relevance being raised by the AI in the pre-prompt interaction. In a sense the model would be used to improve/focus the prompt from the user. We would be making things somewhat more agentic, but there will be no writing updates to files at this point, the user would be required to update their end and re-submit the script overwriting the previous one, in order to update the script the chatbot has. Likewise the documents/scripts attached to given session should be on left attachements panel, with individual buttons representing the attached files, and in same row as button a small delete button, to remove the file from the session, as less files would likely be less confusing for the AI, as are optimal for the user to do so. Possibly we would want individual iterations for interacting with each file, in order to produce reports, that are then fed in as respose, we would break tasks down as are optimal to do so, but obviously we would use, thinking and non-thinking, responses optimally to the applicable tasks, if the request is simple or complete, to then, disable think for simple tasks and enable think for complex/planning tasks. Other settings could also be determined through this auto feature pre-prompt system. Needs brainstorming and planning. Need to ensure attachments is working correctly, and if/when so, then we need artifacts, ie the ability to reference files provided earlier in the session and their presentation. Maybe there is an easier method, also possibly we could just limit to MOE models, and it can do such things more optimally. Sanity needs checking here.
