...
### Current News
- Working on Image Reading, llama.cpp are delaying full supposrt for "Qwen3-VL" in pre-built wheel, this is supposedly a next level model for the 7/8B range with visual and tools to boot...
```
...
```
- All above was delayed, and so found solution is xllamacpp. This will remove need for wheel/model compatibility in wheel, AND speed up responses due to pre-built vulkan wheels option. Working on this now.
