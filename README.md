...
### Status
Beta - Latest release v1.04.x now includes "About/Debug Info" tab with contact/support info. See release notes, last update was fix ESpeak installer issue on Windows 10. Work on the Project has halted for now, so focus can return to other things. It has also become apparent that, the program is so complicated now, that unless I have multiple OS setup, then I cannot bugfix it correctly for intended compatibility ranges, as its a MASSIVE challenge for AI. Possible I could also continue work when AI advances a bit. I will now be focusing on windows 10-11 project restart of Agent-Electron-Gguf, making best use of what I have learned here.
