...
### Current News
- Working on Image Reading, llama.cpp are delaying full supposrt for "Qwen3-VL" in pre-built wheel, will have to test/bugfix with other models
- Working on installer to be download/install , CPU/CPU or VULKAN/CPU or VULKAN/VULKAN, so as to be able to build the wheel for vulkan, while still offering options for VulkanBin+CpuWheel. (must note somewhere that installing with Vulkan Wheel on Windows requires `CMake` on system path).
